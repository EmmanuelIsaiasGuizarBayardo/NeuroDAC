...
def main():
    ...
    while running:
        ...
        # Barra de salud general basada en la flor activa
        f = flores[current]
        h = f['health']
        ...
        # Reinicia si la salud actual llega a 0
        if h == 0:
            flores, completed, current = iniciar_jardin(WIDTH, HEIGHT)
            continue

        # Dibujar flores completadas
        for cf in completed:
            draw_flower(screen, cf)

        # Eventos para flor actual
        ...
        # Comprobar finalización de flor activa
        if f['stage'] == STAGES:
            completed.append(f)
            next_idx = current + 1
            if next_idx < len(flores):
                # Iniciar siguiente flor con salud restante de la previa
        ...
        # Dibujar flor activa e interfaz
        draw_flower(screen, f)
        draw_text(screen, font, f"Flor {current+1}/{MAX_FLOWERS}", WIDTH//2, 70)
        draw_text(screen, font, f"Atención: {f['attention']}", WIDTH//2, 100)
