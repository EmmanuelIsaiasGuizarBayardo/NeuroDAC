...
Levanta el servidor Dash y descubre las paginas de `pages/`. Es el unico
ejecutable del proyecto:

    uv run python interfaz.py
    uv run python interfaz.py --debug        # recarga automatica al editar
    uv run python interfaz.py --host 0.0.0.0 # visible desde otra maquina
"""
...
app.layout = html.Div(
    [
        ...
        dbc.Container(fluid=True, className="py-3", children=[dash.page_container]),
    ]
)
...
# =============================================================
def main() -> None:
    parser = argparse.ArgumentParser(description="Servidor de NeuroDAC.")
    parser.add_argument(
        "--debug",
        action="store_true",
        help=(
            "Recarga automatica al editar. Apagado por omision: el recargador "
            "importa el modulo dos veces, lo que duplica la memoria y puede "
            "abrir el puerto serial dos veces."
        ),
    )
    ...
    # La terminal es el instrumento de diagnostico durante una demostracion:
    # el estado del enlace y los errores del hilo lector salen por aqui.
    logging.basicConfig(
        level=logging.INFO,
    ...
    print(f"NEURODAC en http://{args.host}:{args.port}  ·  CTRL + C para detener")
