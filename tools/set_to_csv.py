"""
set_to_csv.py ÔÇö Convertidor de archivos EEGLAB (.set) a CSV para NEURODAC
DUNNE ┬À Divisi├│n Universitaria de Neuroingenier├¡a

Uso:
    python set_to_csv.py archivo.set [archivo_salida.csv]

Si no se especifica nombre de salida, se genera autom├íticamente.
...
def convert_set_to_csv(input_path, output_path=None):
    ...
    Par├ímetros:
        ...
        output_path (str): Ruta de salida para el CSV. Si es None, se genera autom├íticamente.
    ...
    if not input_path.lower().endswith('.set'):
        raise ValueError("El archivo debe tener extensi├│n .set")
    
    # Generar nombre de salida si no se especific├│
    ...
    try:
        ...
        # Concatenar epochs en una se├▒al continua
        ...
        print(f"  Duraci├│n total: {n_samples / sfreq:.2f} segundos")
    ...
    # --- Determinar si los datos ya est├ín en ┬ÁV o en Volts ---
    # MNE lee en Volts, pero algunos archivos .set ya almacenan en ┬ÁV.
    # Si la media absoluta de los canales es > 1 (en "Volts"), es porque
    # los datos ya estaban en ┬ÁV y MNE no los escal├│ correctamente.
    mean_abs = np.mean(np.abs(data.mean(axis=1)))
    
    if mean_abs > 0.1:  # Los datos ya est├ín en ┬ÁV (MNE los ley├│ sin escalar)
        data_uv = data * 1e6  # MNE asume Volts; si el .set ya tiene ┬ÁV, esto da valores enormes
        ...
        # Datos en Volts (caso normal); convertir a ┬ÁV
        data_uv = data * 1e6
    ...
    # Redondear los valores de se├▒al a enteros (como en el CSV original)
    for col in ch_names:
